...
| `users-table.tsx` | Tableau admin des inscrits (tri, colonnes masquables, édition inline des repas / paiements). Délègue toute la logique de filtrage à `users-filters.tsx`. Les cellules repas d'un non-participant (`isAttending === false`) sont inertes (`—`) : l'API refuse d'y cocher un repas. |
