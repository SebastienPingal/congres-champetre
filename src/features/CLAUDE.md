...
**Layout du dashboard :** les `<X>Section` racine sont des blocs **plats** (`<section className="flex flex-col gap-X">`) — pas de `Card` enveloppante. Le titre de la section est rendu par le dashboard (icône + nom au-dessus). Chaque section commence typiquement par une ligne `description (gauche) + badge de statut (droite)`. Les listes internes (repas, conférences, programme) utilisent `divide-y rounded-lg border bg-surface` plutôt que des cartes encapsulées (le token `--surface` est slightly lighter que `--paper` par thème, donc lisible en light comme en crépuscule).
